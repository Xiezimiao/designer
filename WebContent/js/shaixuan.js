...
oSelectList.onclick = function(e, a) {  //无名函数
    ...
    //一般情况下，增加 
		...
		for (var i = 0; i < aRadio.length; i++) {
            ...
            if(aRadio[i].checked){infor += '<div class=\"selectedInfor selectedShow\"><span>风格</span><label>' + next(aRadio[i]).innerHTML + '</label><em p="2"></em></div>';}
        }
		for (var i = 0; i < bRadio.length; i++) {
			
			if(bRadio[i].checked) {infor += '<div class=\"selectedInfor selectedShow\"><span>户型</span><label>' + next(bRadio[i]).innerHTML + '</label><em p="2"></em></div>';
			}
			
        }
		for (var i = 0; i < cRadio.length; i++) {
           
			if(cRadio[i].checked) infor += '<div class=\"selectedInfor selectedShow\"><span>面积</span><label>' + next(cRadio[i]).innerHTML + '</label><em p="2"></em></div>';;
...
$('div .eliminateCriteria').click(function(){   //点击了 “清空全部”
    ...
    radioVal = '';
   // okSelect.length = 0;
    $(oSelectList).trigger('click', 1);  //调用 onclick（event，1）
})
